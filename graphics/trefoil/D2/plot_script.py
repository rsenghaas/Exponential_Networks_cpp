...
if sys.argv[1] == "all":
    ...
    mask1 = [i for i in range(1430)]
    mask2 = [i for i in range(1470, 1800)]
    x_data_1 = transform(data[mask1, 0])
    x_data_2 = transform(data[mask2, 0])
    plt.plot(x_data_1.real, x_data_1.imag, linewidth=0.8, color=black,
             zorder=2)
    plt.plot(x_data_2.real, x_data_2.imag, linewidth=0.8, color=black,
             zorder=2)
    plt.plot(sing_tf.real, sing_tf.imag, color='white', marker='o', markersize=4,
             fillstyle='full', linestyle='none', mew=0.4)
