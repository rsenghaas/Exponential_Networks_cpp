...
auto ADHM::custom_BPS() -> void {
  ...
  endpoint.at(kIndexY1) -= 4 * pi * J;
  add_new_path(endpoint);
  path_it = get_iterator_by_id(new_paths_, 4);
  evolve_path(path_it, kCutoff);
  save_data(4);

  two_path_intersection_handler(3,4, true, true, -1,0, false, true);
  ...
  two_path_intersection_handler(2,5, true, true, 0,0, false, false);
