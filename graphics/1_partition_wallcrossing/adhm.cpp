...
auto ADHM::custom_BPS() -> void {
  ...
  self_intersection_handler(1, true, -1, 0, true, false);
  save_data(1);
  path_it = get_iterator_by_id(new_paths_, 3);
  evolve_path(path_it, kCutoff);
  ...
  path_it = get_iterator_by_id(new_paths_, 5);
  evolve_path(path_it, kCutoff);
  endpoint = path_it->get_point(11);
  spdlog::debug("Path 5 state");
  print_state_type(endpoint);
  save_data(5);
  path_it = get_iterator_by_id(new_paths_, 6);
  evolve_path(path_it, kCutoff);
  two_path_intersection_handler(5,6, true, true, -3,0, true, false);
  save_data(6);
  ...
  two_path_intersection_handler(1,8,false,true, -2,0,true,true);
  save_data(8);
  ...
  evolve_path(path_it, kCutoff);
  two_path_intersection_handler(1,10,false,true, -1,0, true, true);
  save_data(10);
