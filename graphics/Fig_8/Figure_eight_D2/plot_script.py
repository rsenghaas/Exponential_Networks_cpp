...
def build_truncate(intersection_file, highlight):
    ...
    with open(intersection_file, newline="") as f:
        ...
        for row in reader:
            ...
            lookup[id_new].append((id_A, cutoff_A, id_B, cutoff_B))
...
if os.path.exists('./plot_script.py'):
    ...
    output_dir = 'graphics/Fig_8/' + partition
...
if sys.argv[1] == "all":
    ...
    for k in range(30, 31):
        ...
        for file in glob.glob("*.csv"):
            highlight = []
            path_colors[98] = red
            path_colors[104] = green
            if len(sys.argv[2:]) == 0:
                pass #highlight = []
            else:
                for s in sys.argv[2:]:
                    highlight.append(int(s))
            truncate = {6: -50, 10: -50, 14: -40, 15: -75, 20: -58, 22: -80}
            if False:
