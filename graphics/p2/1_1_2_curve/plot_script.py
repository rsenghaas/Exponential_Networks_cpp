...
path_colors = {0: dark_red, 1:red, 2: light_red,
               3: orange, 4: yellow, 5: light_yellow, 
               6: green_dark, 7: green, 8: green_light, 
               9: black, 10: grey, 11: blue, 
               12: pink}
    #5: red, 7: red}

partition = '1_1_2_curve'
# TODO: Need to change directories, so we can actually run that from the subdirectories.
if os.path.exists('./plot_script.py'):
    output_dir = '.'
else:
    output_dir = 'graphics/p2/' + partition
    pathlib.Path(output_dir).mkdir(parents=True, exist_ok=True)
    if os.path.exists(output_dir + '/data'):
        shutil.rmtree(output_dir + '/data')
    shutil.copytree('data', output_dir + '/data')
    shutil.copy('networks/p2.cpp', output_dir + '/p2.cpp')
    shutil.copy('./plot_p2.py', output_dir + '/plot_script.py')
...
if sys.argv[1] == "all":
    ...
    for file in glob.glob("*.csv"):
        ...
        if i in []:
            order = 4
        plt.plot(x_data.real, x_data.imag, linewidth=0.5, color=color,
                 zorder=order)
