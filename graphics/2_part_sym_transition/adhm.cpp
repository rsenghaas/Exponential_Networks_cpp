...
auto ADHM::custom_BPS() -> void { 
  ...
  evolve_path(path_it, 4*kD4Cutoff);
  path_it->truncate(0, 2400);
  // save_data(0);

  path_it = get_iterator_by_id(new_paths_, 1);
  evolve_path(path_it, 4*kCutoff);
  path_it->truncate(0, 2030);
  save_data(1);
  ...
  endpoint.at(kIndexY1) = endpoint.at(kIndexY2);
  endpoint.at(kIndexY2) += 2 * pi * J;
  add_new_path(endpoint);
  path_it = get_iterator_by_id(new_paths_, 4);
  evolve_path(path_it, 0.5*kD4Cutoff);  
  save_data(4);
  ...
  two_path_intersection_handler(1,5, true, true, 1, 0, true, false);
  save_data(5);
  path_it = get_iterator_by_id(new_paths_, 6);
  ...
  two_path_intersection_handler(3,6, true, true, -2, 0, true, false);
  save_data(3);
  save_data(6);
