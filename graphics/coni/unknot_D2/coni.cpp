...
auto Coni::custom_BPS_trifoil(double cutoff) -> void {
    ...
    two_path_intersection_handler(10, 12, true, true, 0, 0, false, false); 
    save_data(10);
    save_data(12);
    path_it = get_iterator_by_id(new_paths_, 23);
    save_data(path_it->path_id_);
    evolve_path(path_it, cutoff);
    save_data(path_it->path_id_);

    two_path_intersection_handler(13, 23, false, true, 0, 0, false, false); 
    two_path_intersection_handler(7, 13, true, true, 0, 0, false, false); 
    save_data(7);
    save_data(13);
    save_data(23);
    ...
    for (uint32_t l = 0; l < rounds; l++) {
        break;
        uint32_t generation_end = new_paths_.size();
        uint32_t k =generation_end;
