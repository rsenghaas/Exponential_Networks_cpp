...
path_colors = {}
# path_colors = {1: green, 2: grey, 9: grey, 10:red, 12: grey, 13: red}
...
if os.path.exists('./plot_script.py'):
    ...
    output_dir = 'graphics/F_fig_8/' + partition
...
if sys.argv[1] == "all":
    ...
    plt.plot(branch_tf.real, branch_tf.imag, color=orange, marker='x',
             markersize=5,
             fillstyle='none', linestyle='none', mew=1)
    pt = transform(np.array([-0.94, -0.47+0.81j]))
    # plt.plot(pt.real, pt.imag, linestyle='none', marker='o', fillstyle='full', color=blue, markersize=1)
    plt.axis([-1.5, 0.5, -1, 1])
    ax = plt.gca()
