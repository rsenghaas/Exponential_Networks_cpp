...
auto ADHM::backwards(std::vector<uint32_t> pattern_vec) -> void {
  ...
  *path_it = cutoff_path;
  ...
  if (path_2_end_partner_ != 2) {
    path_it = get_iterator_by_id(new_paths_, path_2_end_partner_);
    auto next_state = path_it->get_endpoint();
    path_it = get_iterator_by_id(new_paths_, 2);
    auto endpoint = path_it->get_endpoint();
    endpoint.at(kIndexX) = next_state.at(kIndexX);
    curve_->match_fiber(endpoint);
    path_it->truncate(0, path_2_endtime_);
    path_it->add_single_point(endpoint);
  }
  save_data(2);
}

auto make_circe_line(std::vector<Path>& path_vec, uint32_t path_id) -> void {
  auto path_it = get_iterator_by_id(path_vec, path_id);
  auto v = path_it->get_endpoint();
  v.at(kIndexY1) = v.at(kIndexY2) - 2 * pi * J;
  std::vector<state_type> v_vec = {v};
  std::vector<double> masses = {0};

  Path path(v_vec, masses, path_id);
  *path_it = path;
}
...
auto overwrite_path(std::vector<Path>::iterator path_it, const state_type& v) {
  ...
  Path new_path(v_vec, masses_vec, path_it->path_id_);
  *path_it = new_path;
}
...
auto ADHM::draw_fans() -> void {
  auto path_it = get_iterator_by_id(new_paths_, 1);
  evolve_path(path_it, kCutoff);
  // path_it->truncate(0, 2200);
  save_data(1);

  path_it = get_iterator_by_id(new_paths_, 2);
  evolve_path(path_it, kCutoff);
  path_it->truncate(0, 1700);
  save_data(2);
  ...
  path_it = get_iterator_by_id(new_paths_, 3);
  auto endpoint = path_it->get_endpoint();
  endpoint.at(kIndexY1) = endpoint.at(kIndexY2) - 2 * pi * J;
  ...
  evolve_path(path_it, kCutoff);
  path_it->truncate(0, 1200);
  save_data(4);

  two_path_intersection_handler(1, 4, false, false, -1, 0, false, false);
  path_it = get_iterator_by_id(new_paths_, 5);
  evolve_path(path_it, kCutoff);
  path_it->truncate(0, 1200);
  save_data(5);

  two_path_intersection_handler(2, 5, false, true, 0, 0, false, false);
  save_data(5);
  ...
  path_it = get_iterator_by_id(new_paths_, 5);
  auto basepoint = path_it->get_point(0);
  path_it = get_iterator_by_id(new_paths_, 1);
  endpoint = path_it->get_point(1496);
  endpoint.at(kIndexX) = basepoint.at(kIndexX);
  ...
  path_it = get_iterator_by_id(new_paths_, 3);
  basepoint = path_it->get_point(0);
  path_it = get_iterator_by_id(new_paths_, 1);
  endpoint = path_it->get_point(1550);
  endpoint.at(kIndexX) = basepoint.at(kIndexX);
  endpoint.at(kIndexY2) += 2 * pi * J;
  curve_->match_fiber(endpoint);
  add_new_path(endpoint);
...
auto ADHM::custom_BPS() -> void {
  ...
  startpoint_3.at(kIndexY2) -= 4 * pi * J;
  auto startpoint_4 = endpoint;
  // invert_state(startpoint_4);
  startpoint_4.at(kIndexY2) = startpoint_4.at(kIndexY1) + 4 * pi * J;
  ...
  two_path_intersection_handler(3, 4, true, false, -2, 0, true, false);
  save_data(3);
  save_data(4);
  path_it = get_iterator_by_id(new_paths_, 5);
  evolve_path(path_it, kCutoff);
  ...
  endpoint = path_it->get_endpoint();
   auto startpoint_6 = endpoint;
  // invert_state(startpoint_3);
  startpoint_6.at(kIndexY2) -= 2 * pi * J;
  auto startpoint_7 = endpoint;
  // invert_state(startpoint_4);
  startpoint_7.at(kIndexY2) =  startpoint_7.at(kIndexY1) + 2 * pi * J;

  add_new_path(startpoint_6);
  add_new_path(startpoint_7);
  ...
  two_path_intersection_handler(6,7,true,false,-3, 0,true, false);
  two_path_intersection_handler(0,7,true,true, 0,0,false,false);
  save_data(6);
  save_data(7);
  ...
  */
  two_path_intersection_handler(0, 4, true, true, 0, 0, false, false);
  save_data(4);
  double Z = 0;
  for (uint32_t i = 0; i < 5; i++) {
    path_it = get_iterator_by_id(new_paths_, i);
    Z += path_it->get_endmass();
  }
  spdlog::debug("Central Charge: {}", Z);
  ...
  */
  /*
  two_path_intersection_handler(3,7,true,true,0,0,false,false);
  save_data(3);
  // save_data(7);
  path_it = get_iterator_by_id(new_paths_, 8);
  evolve_path(path_it, kCutoff);
  ...
  path_it = get_iterator_by_id(new_paths_, 5);
  auto startpoint_9 = path_it->get_point(0);
  invert_state(startpoint_9);
  startpoint_9.at(kIndexY1) += 2 * pi * J;
  add_new_path(startpoint_9);
  path_it = get_iterator_by_id(new_paths_, 9);
  evolve_path(path_it, kCutoff);
  path_it->truncate(0, 800); */
  // save_data(9);
  /*
  ...
  */
  ...
  endpoint.at(kIndexX) = 0.5*endpoint.at(kIndexX) +
  0.5*path_it->get_point(18).at(kIndexX);
  ...
  true,false); save_data(current_index); current_index++; path_it =
  get_iterator_by_id(new_paths_, current_index); evolve_path(path_it, kCutoff);
  save_data(current_index);
  ...
  save_data(current_index);
  two_path_intersection_handler(1,current_index, false, true, -1, 0, false,
  false); save_data(current_index); current_index++; path_it =
  get_iterator_by_id(new_paths_, current_index); evolve_path(path_it, kCutoff);
  two_path_intersection_handler(baseline,current_index, true, true, -1, 0, true,
  false); save_data(current_index); save_data(baseline); current_index++;
  path_it = get_iterator_by_id(new_paths_, current_index);
  evolve_path(path_it, kCutoff);
  two_path_intersection_handler(2, current_index, false, true, 0,0,true,false);
  save_data(current_index);
  ...
  evolve_path(path_it, kCutoff);
  two_path_intersection_handler(1,current_index,false, true, -1, 0, false,
  false); save_data(current_index); current_index++;

  path_it = get_iterator_by_id(new_paths_, current_index);
  evolve_path(path_it, kCutoff);


  path_it = get_iterator_by_id(new_paths_, current_index + 1);
  evolve_path(path_it, kCutoff);

  two_path_intersection_handler(current_index + 1, current_index, true, true,
  -1, 0, false, false); save_data(current_index); current_index++;
  save_data(current_index);
  current_index++;
