...
auto Spectral_Network::get_iterator_by_id(std::vector<Path>& path_vec, uint32_t id)
...
auto Spectral_Network::start_paths() -> void {
  for (auto r_it = ramification_points_.begin(); r_it != ramification_points_.end(); r_it++) {
    ...
    start_state.at(kIndexY1) = y;
    start_state.at(kIndexY2) = y;
    ...
    for (uint32_t k = 0; k < branch_n + 1; k++) {
      ...
      next_state.at(kIndexY1) = start_state.at(kIndexY1) + dy;
      next_state.at(kIndexY2) = start_state.at(kIndexY2) + zetaN * dy;
      ...
      spdlog::debug(
          ...
          complex_to_string(std::abs(curve_->eval_H(
              v.back().at(kIndexX), v.back().at(kIndexY1)))),
          complex_to_string(std::abs(curve_->eval_H(
              v.back().at(kIndexX), v.back().at(kIndexY2)))));
...
auto Spectral_Network::initial_integration() -> void {
...
auto Spectral_Network::evolve_path(std::vector<Path>::iterator path_it, double cutoff)
    -> void {
  future_type future = path_it->integrate(curve_, theta_, cutoff);
  path_it->update(std::ref(future));
}

auto Spectral_Network::add_new_path(state_type start_point) -> void {
  std::vector<state_type> v;
...
auto Spectral_Network::add_new_path_with_mass(state_type start_point, double mass) -> void {
    std::vector<state_type> v;
    v.push_back(start_point);
...
auto Spectral_Network::save_data(uint32_t id) -> void {
  auto path_it = get_iterator_by_id(new_paths_, id);
  path_it->save_data();
}

auto Spectral_Network::self_intersection_handler(uint32_t id, bool truncate, int32_t n,
...
auto Spectral_Network::self_intersections(std::vector<Path>::iterator path_it)
...
auto Spectral_Network::two_path_intersections(std::vector<Path>::iterator path_A_it,
...
/* auto Spectral_Network::multiplicity_intersection(uint32_t id_A, uint32_t id_B, bool truncate_A, bool, truncate_B, uint32_t n_a, uint32_t n_B, uint32_t intersection_number) -> void {
...
auto Spectral_Network::two_path_intersection_handler(uint32_t id_A, uint32_t id_B,
...
auto Spectral_Network::intersect_states(state_type state_A, state_type state_B,
...
auto Spectral_Network::compute_intersection_points(intersection& inter,
...
/* auto Spectral_Network::evolution_step() -> void {
...
auto Spectral_Network::handle_new_intersections(
...
auto Spectral_Network::add_new_paths() -> void {
...
auto Spectral_Network::compute_intersection_points_old() -> void {
...
/* auto Spectral_Network::determine_sign(const state_type& r, state_type& v) -> void {
  state_type dv;
...
auto Spectral_Network::print_ramification_points() -> void {
...
auto Spectral_Network::draw_circle(std::vector<state_type> &line, cplx center, std::vector<double> &masses) -> void {
...
auto Spectral_Network::draw_straight(std::vector<state_type> &line, cplx x_end, std::vector<double> &masses) -> void {
    auto x_start = line.back().at(kIndexX);
...
auto Spectral_Network::draw_arc(std::vector<state_type> &line, cplx x_end, std::vector<double> &masses, int32_t winding) -> void {
...
auto Spectral_Network::encircle_points(state_type v, const std::vector<cplx> &way_points, std::vector<double> &masses, cplx offset) -> std::vector<state_type> {
...
auto Spectral_Network::circle_probe(const cplx &x0, const cplx &anker) -> void {
...
auto Spectral_Network::encircle_probe(const cplx &x0, const cplx &x1) -> void {
...
auto Spectral_Network::ramification_probe(const cplx &x0) -> void {
    auto fiber = curve_->get_fiber(x0);
    state_type v;
    for(auto& r : ramification_points_) {
...
auto Spectral_Network::two_point_probe(const cplx &x_start, const cplx &x_end) -> void {
...
auto Spectral_Network::probe_curve() -> void { 
    cplx anker = 0.8;
