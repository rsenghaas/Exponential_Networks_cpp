...
auto eval_ex_to_complex(const GiNaC::ex &f, const GiNaC::symbol &x, const cplx &z) -> cplx {
    return numeric_to_complex(GiNaC::ex_to<GiNaC::numeric>(f.subs({x == complex_to_ex(z)})));
}
...
auto roots(const GiNaC::ex &f, const GiNaC::symbol &x) -> std::vector<cplx> {
    ...
    std::generate(std::begin(approx_roots), std::end(approx_roots), [i = 0] () mutable {return numeric_to_complex(GiNaC::ex_to<GiNaC::numeric>(GiNaC::pow(complex_to_ex(kDummyRoot), i++)));});
    ...
    do {
        ...
        for (uint32_t i = 0; i < deg; i++)
        {
            ...
            for(uint32_t j = 0; j < deg; j++) {
                if(j != i) {
                    // std::cout  << "(" << i << "," << j << ")" << "->" << (approx_roots.at(j) - approx_roots.at(i)) << std::endl; 
            ...
            norms.at(i) = std::abs(eval_ex_to_complex(Poly, x, new_approx_roots.at(i)));
        }
        approx_roots = new_approx_roots;
        max_norm = *std::max_element(std::begin(norms), std::end(norms));
        iteration_counter++;
    } while (max_norm > kZerosPrecisions && iteration_counter < kZerosMaxIterations);
    return approx_roots;
}
