...
class ODE_integrator {
  public:
    ODE_integrator(ODE_differential diff,  state_type v0) :  diff_(diff), v0_(v0), t0_(0), obs_() {
        obs_.times.push_back(t0_);
        obs_.masses.push_back(0);
        obs_.states.push_back(v0);
    }

    auto integrate_ode() -> void;
    auto print_v() -> void;
...
struct observer {
  ...
  void operator()(const state_type &v, const double t) {
    state_type dv;
    std::transform(std::begin(v), std::end(v), std::begin(m_obs.states.back()), std::begin(dv), [](std::complex<double> v1, std::complex<double> v2){return v1 - v2;});
